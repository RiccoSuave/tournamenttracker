...
namespace TrackerLibrary
{
    public static class TournamentLogic
    {
        ...
        //Check if it is big enough, and if it is not add in byes - 2*2*2*2 - 2^4
        ...
        private static decimal CalculatePrizePayout (this PrizeModel prize, decimal totalIncome)
        {
            ...
            {
                output = Decimal.Multiply(Convert.ToDecimal( prize.PrizePercentage/100 ), totalIncome);
            }
            return output;
        ...
        private static void MarkWinnersInMatchups(List<MatchupModel> models)
        {
            ...
            // zero means false or low score wins 
            foreach (MatchupModel m in models)
            {
                ...
                else
                {
                    ...
                    if ((m.Entries.Count == 1) || (m.Entries[0].Score > m.Entries[1].Score))
                    {
                        m.Winner = m.Entries[0].TeamCompeting;
                    }
                    else if (m.Entries[1].Score > m.Entries[0].Score)
